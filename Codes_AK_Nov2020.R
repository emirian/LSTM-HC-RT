...
########################### Load All ##########################
subnames = c("Alpha","Beta","Delta","Gamma","Theta")
dat = NULL
sheetNames = excel_sheets("TrainOnFull_Results4AK.xlsx")
for(ShName in sheetNames){
        temp      = read_xlsx("TrainOnFull_Results4AK.xlsx", sheet = ShName)
        temp = melt(temp, id.vars = c("Trial"), variable.name = "ID",value.name = "DeltaRT")
        temp$Train = unique("Full")
        temp$Test = unique(ShName)
        dat = rbind(dat,temp)
}
sheetNames = excel_sheets("TrainOnSB_Results4AK.xlsx")
i=1
for(ShName in sheetNames){
        temp      = read_xlsx("TrainOnSB_Results4AK.xlsx", sheet = ShName)
        temp = melt(temp, id.vars = c("Trial"), variable.name = "ID",value.name = "DeltaRT")
        # temp$Train = unique(subnames[i])
...
write.csv(trialLevelDat,"AK_TriallevelData.csv",row.names = FALSE)

subjectLevelDat = read.csv("AKsort_Meanvalues.csv",sep = ",",header=TRUE,strip.white=TRUE)
subjectLevelDat$Train = as.character(subjectLevelDat$Train)
subjectLevelDat$Train[subjectLevelDat$Train!="Full"] = unique("Subband")
subjectLevelDat$Train = as.factor(subjectLevelDat$Train)                        
...
ggplot(subjectLevelDat,aes(x=Train, y=MSE, fill = Train)) + 
        geom_bar(stat="summary",fun.y="mean",position="dodge")+
        stat_summary(fun.data = "mean_se", geom="errorbar",position="dodge")+
        facet_wrap(~Test)+
        theme(strip.text.x = element_text(size=16, face="bold"))+
        labs(x="Trained on",y="", size=16)+
        theme(axis.title.y = element_text(size = 18))+
        theme(axis.text.x = element_text(size = 16))+
        theme(strip.text.x = element_text(size=16, face="bold"),
              strip.text.y = element_text(size=16, face="bold"))+
...
# Anovadat = trialLevelDat[complete.cases(trialLevelDat),]
# Anovadat = Anovadat[Anovadat$Train=="Full",]
# Anovadat = Anovadat[Anovadat$Train=="Subband",]
# Anovadat$DeltaRT = abs(Anovadat$DeltaRT)
# results=as.data.frame(ezANOVA(data=Anovadat, dv=DeltaRT,wid=.(ID),within=.(Test)
...
# 
# Anovadat = trialLevelDat[complete.cases(trialLevelDat),]
# results=as.data.frame(ezANOVA(data=Anovadat, dv=DeltaRT,wid=.(ID),within=.(Test,Train)
#                               ,type=3,detailed=T)$ANOVA)
# results$pareta=results$SSn/(results$SSn+results$SSd)
# is.num=sapply(results, is.numeric)
# results[is.num] =lapply(results[is.num], round, 3)
...
Plotdat = subjectLevelDat[subjectLevelDat$Train=="Full",]
ggplot(Plotdat,aes(x=Test, y=MSE, fill = Test)) + 
        geom_bar(stat="summary",fun.y="mean",position="dodge")+
        stat_summary(fun.data = "mean_se", geom="errorbar",position="dodge")+
        # facet_wrap(~Train)+
        theme(strip.text.x = element_text(size=16, face="bold"))+
        labs(x="Tested on",y="", size=16)+
        ggtitle("Subject Level Data. Trained on Full")+
        theme(axis.title.y = element_text(size = 18))+
        theme(axis.text.x = element_text(size = 16))+
        theme(strip.text.x = element_text(size=16, face="bold"),
              strip.text.y = element_text(size=16, face="bold"))+
...
Plotdat = subjectLevelDat[subjectLevelDat$Train=="Subband",]
ggplot(Plotdat,aes(x=Test, y=MSE, fill = Test)) + 
        geom_bar(stat="summary",fun.y="mean",position="dodge")+
        stat_summary(fun.data = "mean_se", geom="errorbar",position="dodge")+
        # facet_wrap(~Train)+
        ggtitle("Subject Level Data. Trained on subbandl")+
        theme(strip.text.x = element_text(size=16, face="bold"))+
        labs(x="Tested on",y="", size=16)+
        theme(axis.title.y = element_text(size = 18))+
        theme(axis.text.x = element_text(size = 16))+
        theme(strip.text.x = element_text(size=16, face="bold"),
              strip.text.y = element_text(size=16, face="bold"))+
        stat_compare_means( comparisons = list(c("Gamma","Theta"),
                                               c("Beta","Theta")),
                            label = "p.signif",label.y = c(.025,.028,.03))
...
subnames = c("Alpha","Beta","Delta","Gamma","Theta")
dat = NULL
sheetNames = excel_sheets("TrainOnFull_Results4AK_Vigour.xlsx")
for(ShName in sheetNames){
        temp      = read_xlsx("TrainOnFull_Results4AK.xlsx", sheet = ShName)
        temp = melt(temp, id.vars = c("Trial"), variable.name = "ID",value.name = "DeltaRT")
        temp$Train = unique("Full")
...
sheetNames = excel_sheets("TrainOnSB_Results4AK_Vigour.xlsx")
i=1
for(ShName in sheetNames){
        temp      = read_xlsx("TrainOnSB_Results4AK.xlsx", sheet = ShName)
        temp = melt(temp, id.vars = c("Trial"), variable.name = "ID",value.name = "DeltaRT")
...
subjectLevelDat = NULL
temp = read_xlsx("Results_Vigour.xlsm",sheet = 3)
temp = melt(temp, id.vars = c("ID"), variable.name = "Test",value.name = "MSE")
temp$Train = unique("Subband")
subjectLevelDat = rbind(subjectLevelDat,temp)
temp = read_xlsx("Results_Vigour.xlsm",sheet = 5)
temp = melt(temp, id.vars = c("ID"), variable.name = "Test",value.name = "MSE")
temp$Train = unique("Full")
subjectLevelDat = rbind(subjectLevelDat,temp)
subjectLevelDat = subjectLevelDat[,c("ID","Train","Test","MSE")]
subjectLevelDat$MSE = as.numeric(subjectLevelDat$MSE)
...
ggplot(Plotdat,aes(x=Test, y=MSE, fill = Test)) + 
        ...
        theme(strip.text.x = element_text(size=16, face="bold"),
              strip.text.y = element_text(size=16, face="bold"))+
        stat_compare_means( comparisons = list(c("Beta","Full"),
                                               c("Beta","Theta"),
                                               c("Theta","Full"),
                                               c("Gamma","Theta")),
                            label = "p.signif",label.y = c(380,410,430,450))
